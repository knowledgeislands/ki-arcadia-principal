...
Knowledge Islands adopts **Decision Records (DRs)** as the standard instrument for recording significant standalone decisions. The unified `knowledgeislands-decision-records` skill governs the format across all repo types. DRs use the Nygard five-section format with a `decision_type` field drawn from a nine-value taxonomy. Each type has its own prefix — `GDR-` (governance), `ADR-` (architecture), `SDR-` (strategy), `PDR-` (product), `KDR-` (knowledge), `DDR-` (data), `XDR-` (security), `ODR-` (operations), `RDR-` (research) — and its own monotonically increasing serial within a scope.

In KB repos, DRs are placed at `Admin/Decisions/`. Code repos place them at `docs/decisions/`. The `knowledgeislands-adrs` and `knowledgeislands-kdrs` skills are retired.
