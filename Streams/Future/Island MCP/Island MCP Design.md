...
This inverts the current model. Today, tools connect to the island - Claude reads files, automations run scripts against the file tree, and conventions like [[Notes]] and [[Knowledge Capital/Conventions/Routing Rules|Routing Rules]] are honoured by convention rather than by enforcement. With the proposal, agents connect to the island's MCP, and the MCP mediates and enforces all access. The file tree continues to exist - Obsidian and humans still touch it directly - but agent traffic is routed through the gateway.
