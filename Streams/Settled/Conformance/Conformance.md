...
- [x] Add Adoption Requirements to Tending, Briefings, and Linear group indexes
...
**What the stream delivered.** The constitutional layer is in place. The Charter exists as Arcadia's authoritative declaration of identity and adoption state, and the Conformance Check is an operational activity that verifies the island against it. The adoption model is now explicit: every activity group must carry an `adopted` or `vetoed` position in KC, and Adoption Requirements sections in group index notes make the contract legible without hunting through wikilinks.

**What went well.** The two-concept framing — constitutional (Charter + Conformance) vs. adoptable (everything else) — proved clean and durable. Once that distinction was established, the subsequent work (veto stubs, Adoption Requirements, Layer 5 prompt) followed naturally. The checklist covered the ground without requiring significant revision mid-stream.

**Issues encountered.** The proposal was worked informally without the status being progressed through the lifecycle stages. The stream sat at `draft` in the index even after all rollout items were ticked. No knowledge was lost, but the index was inaccurate.

**Lesson.** Ticking off the final checklist item should be an immediate prompt to progress status to `rolled-out`. This is a discipline issue, not a process gap — the process already requires the index to be kept current.
