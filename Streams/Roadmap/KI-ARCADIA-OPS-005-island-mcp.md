...
A roadmap record to design and propose an MCP server that fronts each Knowledge Island - all read and write operations go through the MCP rather than direct file access. The record tracks design questions, dependencies, and the path to a ratified proposal.
...
| Type        | Detail                                                                                              |
| ----------- | --------------------------------------------------------------------------------------------------- |
| Design | Gateway rationale, tool-surface sketch, and open questions retained in this record |
...
The gateway would make agent access enforceable rather than conventional: it could validate note construction at write time, apply role-scoped permissions, record audit evidence, and hide the storage layout behind a stable tool surface. Humans and Obsidian may continue to access the file system directly; the design governs agent traffic.

The initial tool-surface sketch is `read_note`, `search`, `list_folder`, `write_note`, `validate_routing`, `link_check`, and `get_metadata`. The work must still decide whether servers are per-island or shared, whether enforcement is strict at write time or advisory at read time, and how the MCP relates to existing Cowork configuration.
