...
updated_at: 2026-09-20T12:19:28Z
---
...
Re-verify the evidence underpinning the Knowledge Islands factorisation roadmap, establish its repository-structure taxonomy, and prepare the exact in-place amendment and projection set for `GDR-KI-FUNDAMENTALS-001`.
...
This proposal covers read-only verification of the original 22-repository pre-merge estate, the newly created ungoverned `tools-techne` repository, and preparation of the Arcadia-owned amendment plan. It may identify precise receiving repositories, base structures, structural overlays, local work adapters, evidence corrections, amendment outline, and projection checks.
...
The structure review now records Project and Knowledge Base as the only base structures, treats specialist `ki-repo-*` skills as composable overlays, and separates adapters and estate roles. The live inventory is 23 before the OpenAI merge and 22 after it because `tools-techne` appeared after the original evidence pass.
...
- [x] Audit current repository structures and separate base structures, overlays, adapters, estate roles, runtime ownership, and boundary evidence.
- [x] Add `tools-techne` to the inventory and correct the post-merge receiving set to 22 repositories.
...
- Check the 22-repository post-merge register against the two-base structure rule and current structural overlays.
