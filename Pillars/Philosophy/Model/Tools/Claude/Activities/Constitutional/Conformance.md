...
**For each adopted group:** Read the file path given in the Knowledge Capital column (the group's KC index note). Then read the corresponding group index note in `Pillars/Philosophy/Model/Activities/[Group]/[Group].md`. Find the Adoption Requirements section in that note and extract the list of required KC note paths.
