...
2. A proposal folder and note are created in Streams under the appropriate Focus and Category (`Streams/$Focus/$Category/$ProposalName/`) and a row is added to the proposals index
...
Rollout means executing the operations from the Checklist. It is not complete until:
...
- **Keep the proposal document current.** The proposal document is the canonical source for any in-progress change. Update it immediately when a decision is made; it must never lag behind the current state of the change.
...
- **Check prerequisites before marking ready.** A change may not move to `ready` while any prerequisite has not reached `rolled-out` status.
- **Do not begin rollout without explicit authorisation.** Rollout steps must not be executed until the user explicitly instructs it. The `ready` status is a necessary condition, not a sufficient one. Exploratory language ("let's look at this", "let's work through it") is iteration, not authorisation.
