...
The tools agents use are documented separately in [[Model/Tools/Tools|Tools]]. The distinction matters: `Agents/` covers operating behaviour; `Tools/` covers configuration and connection. A given AI system appears in both — once for how it acts, once for how it is set up.
...
The Claude agent note covers the operating conventions specific to Claude — how Claude reads and writes island content, the memory architecture it uses across sessions, the skill system, and the behavioural conventions that make Claude a reliable island agent.

## ChatGPT

The ChatGPT agent note covers the operating conventions for ChatGPT as an island agent — the tasks it handles, how it differs from Claude in its access patterns, and the conventions that govern its use.
