...
Island configuration: [[Pillars/Knowledge Capital/Manifest|Manifest]], [[Pillars/Knowledge Capital/Identity|Identity]]
...
1. `Pillars/Knowledge Capital/Manifest.md` — the island's adoption record and activity roster
2. `Pillars/Knowledge Capital/Identity.md` — the island's identity record
3. `Pillars/Knowledge Islands/Governance/Activities/Activities.md` — the full list of activity groups defined in the framework

### Step 1 — Constitutional baseline

Check each of the three constitutional elements in turn.

**Identity.** The file `Pillars/Knowledge Capital/Identity.md` must exist and contain at minimum: an island name, a KB skill identifier, and a task prefix or equivalent. Flag as CRITICAL if the file is absent, empty, or missing any of these fields.

**Manifest.** You have already read this. Verify it contains: an Activity Groups table with at least one row carrying a clear `adopted` or `vetoed` position; a Scheduled Activities table; and a Tools section. Flag as CRITICAL if any section is absent or the Activity Groups table is empty.

**Conformance.** Check that the Conformance Check appears in the Manifest's Scheduled Activities table with status `enabled`. Flag as CRITICAL if it is absent or carries any other status.
...
For each remaining group, check whether the Manifest's Activity Groups table contains a row for that group with either `adopted` or `vetoed` in the Position column.
...
- Absent from the Manifest entirely → flag as non-conformant: unknown

### Step 3 — Adoption consistency

Work through each group in the Manifest's Activity Groups table.
...
- [[Pillars/Knowledge Capital/Manifest|Manifest]] - primary input
