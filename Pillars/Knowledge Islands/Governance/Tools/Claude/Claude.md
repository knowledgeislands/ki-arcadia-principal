...
**Knowledge Islands routing convention:** the `memory/` directory is a skill convention, not an island convention. If a `memory/` folder is created during a session, it should be treated as a staging area, not a permanent home. Consult [[Routing Rules|Routing Rules]] for the correct destination paths in this KI, then migrate content and remove the folder.
