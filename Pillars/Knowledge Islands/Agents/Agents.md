...
The tools agents use are documented separately in [[Knowledge Islands/Tools/Tools|Tools]]. The distinction matters: `Agents/` covers operating behaviour; `Tools/` covers configuration and connection. A given AI system appears in both - once for how it acts, once for how it is set up.
...
[[Agents/ChatGPT/ChatGPT|ChatGPT]] operates as a read-heavy participant rather than a full agent. Context is loaded manually by the user; outputs are routed into the island by the user rather than by the model directly. It runs no scheduled automations and has no direct write access. It is documented here for structural consistency - if it is ever given direct write access or scheduled tasks, this note expands into a full agent layer.

---

## Related Topics

- [[Pillars/Knowledge Islands/Knowledge Islands|Knowledge Islands]] - the KI framework index; Agents is one of its five governance areas
- [[Pillars/Knowledge Islands/Tools/Tools|Tools]] - the configuration counterpart; where each tool's connection setup and operating conventions live
- [[Pillars/Knowledge Islands/Activities/Authoring Activities|Authoring Activities]] - the five-layer model that defines which layer agent content belongs to
