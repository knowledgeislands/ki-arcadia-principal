...
- **Lessons live in auto-memory** — all resolved lessons from [[Mistakes and Lessons]] are extracted to the relevant `feedback_*` memory files. The lessons are active in every session at zero marginal cost; the island note is a human-readable register, not a machine-read file.
...
**Knowledge Islands routing convention:** the `memory/` directory is a skill convention, not an island convention. If a `memory/` folder is created during a session, it should be treated as a staging area, not a permanent home. Consult [[Routing Rules]] for the correct destination paths in this island, then migrate content and remove the folder.
...
- [[Tools]] — parent index
- [[Agents/Claude/Claude|Agents/Claude]] — the agent layer: five modes, behavioural constraints, memory architecture
- [[Obsidian]] — primary interface used alongside the Claude integration
